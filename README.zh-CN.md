...
本节将创建器控制技能运行的关键点集中到一张人类可读的地图中。`SKILL.md` 与其引用的契约仍是机器执行时的权威规则；这里用于在创建或运行工作流前快速理解边界、证据和交接关系。
...
当当前时期仍为 1 时，“开始”“继续”“执行”等词默认有歧义，应解释为继续完善 skill。只有用户明确表达进入运行期、执行正式 Do/Check 或正式采集时，才允许进入时期 2。
